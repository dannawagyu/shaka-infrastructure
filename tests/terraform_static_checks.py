...
def main() -> int:
    ...
    assert_contains(tf, r'data\s+"aws_ami"\s+"ubuntu_noble"', "Production EC2 must be able to resolve the current-region Ubuntu 24.04 AMI")
    assert_contains(tf, r'owners\s*=\s*\[\s*"099720109477"\s*\]', "Ubuntu AMI lookup must be restricted to Canonical's AWS account")
    assert_contains(tf, r'ubuntu/images/hvm-ssd-gp3/ubuntu-noble-24\.04-amd64-server-\*', "Ubuntu AMI lookup must target Ubuntu 24.04 amd64 server images")
    assert_contains(tf, r'resource\s+"aws_instance"\s+"app"', "Terraform-managed EC2 app instance resource is required")
    assert_contains(tf, r'ami\s*=\s*coalesce\(var\.app_ami_id\s*!=\s*""\s*\?\s*var\.app_ami_id\s*:\s*null,\s*data\.aws_ami\.ubuntu_noble\.id\)', "EC2 AMI must use a valid pinned value or fall back to the current-region Ubuntu lookup")
