...
def main() -> int:
    ...
    assert_contains(alb, r'delivery\.logs\.amazonaws\.com', "ALB access log bucket policy must use the ELB log delivery service principal so aws:SourceAccount applies")
    assert_not_contains(alb, r'783225319266', "ALB access log bucket policy must not use the legacy ELB account-id principal (incompatible with aws:SourceAccount)")
    ...
    assert_contains(alb, r'aws:SourceAccount', "ALB access log bucket policy must enforce aws:SourceAccount to prevent cross-account log injection")
