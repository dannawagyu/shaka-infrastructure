...
def main() -> int:
    ...
    assert_contains(tf, r'variable\s+"existing_app_instance_id"', "Production Terraform must require the existing Shaka EC2 app instance ID")
    assert_contains(tf, r'variable\s+"existing_app_security_group_id"', "Production Terraform must require the existing Shaka app security group ID")
    assert_contains(tf, r'data\s+"aws_instance"\s+"existing_app"', "Production Terraform must reference the existing app EC2 instance")
    assert_contains(tf, r'instance_id\s*=\s*var\.existing_app_instance_id', "Existing app data source must use existing_app_instance_id")
    assert_contains(tf, r'data\s+"aws_subnet"\s+"existing_public"', "Production Terraform must discover the existing app subnet")
    assert_not_contains(tf, r'resource\s+"aws_instance"\s+"app"', "Production Terraform must not create a replacement EC2 app host")
    assert_not_contains(tf, r'resource\s+"aws_vpc"\s+"shaka"', "Production Terraform must not create a replacement VPC while the fixed-IP app server remains canonical")
    assert_not_contains(tf, r'resource\s+"aws_internet_gateway"\s+"shaka"', "Production Terraform must not create replacement public networking for the app host")
    assert_not_contains(tf, r'resource\s+"aws_subnet"\s+"app_public"', "Production Terraform must not create a replacement app subnet")
    assert_contains(tf, r'resource\s+"aws_subnet"\s+"rds_private"', "Terraform-managed private RDS subnets are required")
    assert_contains(tf, r'vpc_id\s*=\s*data\.aws_subnet\.existing_public\.vpc_id', "RDS subnets/security group must be created in the existing app VPC")
    ...
    assert_contains(tf, r'^\s*engine_version\s*=\s*var\.db_engine_version\b', "RDS MySQL engine version must come from a guarded variable")
    assert_contains(tf, r'variable\s+"db_engine_version".*default\s*=\s*"8\.0"', "RDS MySQL engine version must default to the region-supported MySQL 8.0 family")
    ...
    assert_contains(tf, r'resource\s+"aws_security_group"\s+"rds"', "Terraform-managed RDS security group is required")
    assert_contains(tf, r'resource\s+"aws_security_group_rule"\s+"rds_ingress_from_existing_app_ec2".*source_security_group_id\s*=\s*var\.existing_app_security_group_id', "RDS ingress must reference the existing app EC2 security group")
    ...
    assert_contains(outputs, r'output\s+"alb_dns_name"', "Output alb_dns_name is required for the production ALB cutover")
    assert_contains(outputs, r'output\s+"alb_zone_id"', "Output alb_zone_id is required for any future Route53 alias migration")
    assert_contains(outputs, r'output\s+"alb_access_logs_bucket"', "Output alb_access_logs_bucket is required")
    assert_contains(outputs, r'output\s+"acm_certificate_validation_records"', "Output acm_certificate_validation_records is required so the validation CNAME can be added in the external DNS provider")
    assert_not_contains(outputs, r'(password|secret|username)', "Outputs must not expose credentials or secrets")

    alb = read(PROD / "alb.tf")
    assert_contains(alb, r'resource\s+"aws_security_group"\s+"alb"', "Terraform-managed ALB security group is required")
    assert_contains(alb, r'resource\s+"aws_security_group_rule"\s+"app_ingress_from_alb".*security_group_id\s*=\s*var\.existing_app_security_group_id.*source_security_group_id\s*=\s*aws_security_group\.alb\.id', "App SG must accept HTTP only from the ALB SG")
    assert_contains(alb, r'resource\s+"aws_acm_certificate"\s+"alb".*validation_method\s*=\s*"DNS"', "ALB ACM certificate must use DNS validation")
    assert_contains(alb, r'resource\s+"aws_acm_certificate_validation"\s+"alb".*timeouts\s*\{[^}]*create\s*=\s*"60m"', "ACM certificate validation must allow up to 60 minutes for manual DNS CNAME entry")
    assert_contains(alb, r'resource\s+"aws_lb"\s+"shaka"', "Production ALB resource is required")
    assert_contains(alb, r'load_balancer_type\s*=\s*"application"', "Production load balancer must be an Application Load Balancer")
    assert_contains(alb, r'internal\s*=\s*false', "Production ALB must be internet-facing")
    assert_contains(alb, r'enable_deletion_protection\s*=\s*true', "Production ALB must enable deletion protection")
    assert_contains(alb, r'drop_invalid_header_fields\s*=\s*true', "Production ALB must drop invalid header fields")
    assert_contains(alb, r'desync_mitigation_mode\s*=\s*"defensive"', "Production ALB must keep at least defensive desync mitigation")
    assert_contains(alb, r'access_logs\s*\{[^}]*bucket\s*=\s*aws_s3_bucket\.alb_access_logs\.id[^}]*enabled\s*=\s*true', "Production ALB must enable access logs to the Terraform-managed bucket")
    assert_contains(alb, r'resource\s+"aws_s3_bucket"\s+"alb_access_logs"', "ALB access log S3 bucket is required")
    assert_contains(alb, r'resource\s+"aws_s3_bucket_public_access_block"\s+"alb_access_logs".*block_public_acls\s*=\s*true.*block_public_policy\s*=\s*true.*ignore_public_acls\s*=\s*true.*restrict_public_buckets\s*=\s*true', "ALB access log bucket must block all public access paths")
    assert_contains(alb, r'resource\s+"aws_s3_bucket_server_side_encryption_configuration"\s+"alb_access_logs".*sse_algorithm\s*=\s*"AES256"', "ALB access log bucket must encrypt with AES256")
    assert_contains(alb, r'resource\s+"aws_s3_bucket_lifecycle_configuration"\s+"alb_access_logs".*expiration\s+\{[^}]*days\s*=\s*30', "ALB access log bucket must expire objects after 30 days")
    assert_contains(alb, r'resource\s+"aws_s3_bucket_ownership_controls"\s+"alb_access_logs".*object_ownership\s*=\s*"BucketOwnerEnforced"', "ALB access log bucket must enforce bucket-owner object ownership")
    assert_contains(alb, r'DenyInsecureTransport', "ALB access log bucket policy must deny non-TLS access")
    assert_contains(alb, r'logdelivery\.elasticloadbalancing\.amazonaws\.com', "ALB access log bucket policy must use the ELB-specific log delivery service principal (NOT delivery.logs.amazonaws.com which is for CloudWatch Logs/NLB)")
    assert_not_contains(alb, r'783225319266', "ALB access log bucket policy must not use the legacy ELB account-id principal")
    assert_not_contains(alb, r'identifiers\s*=\s*\[\s*"delivery\.logs\.amazonaws\.com"', "ALB access log bucket policy must not use delivery.logs.amazonaws.com as a principal (that is for CloudWatch Logs/NLB and is rejected by ALB)")
    ...
    assert_contains(alb, r'resource\s+"aws_cloudwatch_metric_alarm"\s+"alb_unhealthy_host"', "ALB unhealthy host CloudWatch alarm is required")
    assert_contains(alb, r'resource\s+"aws_cloudwatch_metric_alarm"\s+"alb_5xx"', "ALB 5xx CloudWatch alarm is required")
